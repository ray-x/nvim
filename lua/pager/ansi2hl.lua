...
function state:clear()
  ...
  self.ctermfg = ''
  self.ctermbg = ''
  for _, k in pairs(attributes) do
    self[k] = false
  end
end
...
function state:state2highlight_group_name()
  if self.conceal then
    return 'NvimPagerConceal'
  end
  local name = 'NvimPagerFG_'
    .. self.foreground:gsub('#', '')
    .. '_BG_'
    .. self.background:gsub('#', '')
...
function state:parse(string)
  for token, c1, c2, c3 in tokenize(string) do
    ...
    elseif c1 ~= nil then
      ...
      if token == 0 then
        self:clear()
      elseif token == 1 or token == 3 or token == 4 or token == 7 or token == 8 or token == 9 then
      ...
      elseif token == 22 then
        self.bold = false
      --self.faint = false
      elseif token == 23 or token == 24 or token == 27 or token == 28 or token == 29 then
...
local function ansi2highlight()
  ...
  nvim.nvim_set_option_value('conceallevel', 3, { win = winnr })
  nvim.nvim_set_option_value('concealcursor', 'nv', { win = winnr })
